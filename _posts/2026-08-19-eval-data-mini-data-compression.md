...
这篇文章把"实践 + 调研"放在一起，试图沉淀出一套**评测数据集压缩范式**：让评测团队在不同场景诉求下，能快速判断该选哪一种压缩算法。
...
对**全集跑完整评测**，把每个样本的"多模型得分向量"当作特征，用 K-Means 聚类找到"代表性样本"。直觉：每个簇代表一类"难度/行为画像"，从每个簇中心取点即可近似全集的得分分布。

**说人话就是：** 用历史评测得分做聚类，从每个"难度画像"簇里抽代表——好懂、可解释，但得先跑完全集评测。
...
把每个样本"蒸馏"成特征模型**最后一层隐藏状态**的一个向量，在 RBF 核的再生核希尔伯特空间（RKHS）里，用贪心 Herding 选子集，使子集与全集的**最大均值差异（MMD）**最小化——即让子集在特征空间中的"分布重心"逼近全集。
...
那么，业内同行是怎么解决这个问题的？带着这个疑问，我们把视野转向外部，按"由浅入深、由间接目标到直接目标"的顺序调研了三套方案：从最朴素的 PPL 分层，到统计严谨的 MetaBench，再到直接优化排名的 EssenceBench。
